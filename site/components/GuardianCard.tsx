...
function Spinner() {
  return (
    <svg className={s.spinSvg} width="15" height="15" viewBox="0 0 15 15" fill="none">
      <circle cx="7.5" cy="7.5" r="5.5" stroke="rgba(148,255,194,0.15)" strokeWidth="2" />
      <circle cx="7.5" cy="7.5" r="5.5" stroke="#94FFC2" strokeWidth="2"
...
function Check({ result }: { result: 'pass' | 'warn' | 'fail' }) {
  const c = result === 'pass' ? '#94FFC2' : result === 'warn' ? '#F0B33C' : '#E5524F'
  const f = result === 'pass' ? 'rgba(148,255,194,0.12)' : result === 'warn' ? 'rgba(240,179,60,0.12)' : 'rgba(229,82,79,0.12)'
  return (
      ...
      <div className={`${s.sonarSlot} ${txVisible ? s.sonarSlotHidden : ''}`}>
        <div className={s.sonarWrap}>
          ...
          <div className={s.sonarCenter}>
            <Image src="/assets/sage-mark-mint.png" alt="" width={22} height={22} style={{ opacity: 0.85 }} />
          </div>
        </div>
        <p className={s.sonarLabel}>Sage is monitoring</p>
      </div>
